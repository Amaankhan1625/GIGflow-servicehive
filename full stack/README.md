...
| Bids | GET | /api/bids/user | Get user's own bids. |
| Bids | POST | /api/bids | Submit a bid for a gig. |
| Bids | GET | /api/bids/:gigId | Get all bids for a specific gig (Owner only). |
| Bids | PUT | /api/bids/:bidId | Update a bid (Freelancer only, if pending). |
| Bids | DELETE | /api/bids/:bidId | Delete a bid (Freelancer only, if pending). |
